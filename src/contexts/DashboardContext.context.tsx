...
export default function DashboardContextProvider({ children }: Props) {
	...
	useEffect(() => {
		if (!session) return
		if (session.user.id === process.env.NEXT_PUBLIC_ADMIN_USERNAME) return
	...
	// Set authorization for Ei8 Secret Admin
	useEffect(() => {
		if (!session) return

		if (session.user.id === process.env.NEXT_PUBLIC_ADMIN_USERNAME) {
			const payload: Auth = {
				...session.user,
				userRole: {
					id: process.env.NEXT_PUBLIC_ADMIN_USERNAME,
